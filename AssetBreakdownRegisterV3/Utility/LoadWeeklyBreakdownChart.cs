...
{
    public class LoadWeeklyBreakdownChart
    {
        CultureInfo myCI;
        Calendar myCal;
        ...
        List<Dictionary<string, int>> chartData;

        public LoadWeeklyBreakdownChart(Chart chart, SqliteDataAccess sqlData) {
            this.chart = chart; 
        ...
        public void Load()
        {
            ...
            this.chartData = new List<Dictionary<string, int>>();
            /*example of the data structure
             * [{"WeekOfYear": 5,            
             * "NumberOfBreakdowns": 25
             * },
             * {"WeekOfYear": 6,
             *  "NumberOfBreakdowns": 30
             * },
             * ....
               ]    
            */
            ...
            do
            {
                ...
                //assign to the chartData "counter" index the key week of the year and the nº of breakdown to be later increased
                chartData.Insert(counter, new Dictionary<string, int>()
                {
            ...
            foreach (BreakdownModel breakdown in sqlData.breakdown_list)
            {
                ...
                {
                    //if this breakdown is within the range then update the NumberOfBreakdowns in the chartData (in the correct key, checked by the next if statement, based on the week)
                    foreach (var key in chartData)
                    {                        
                        if (key["WeekOfYear"] == breakdownWeekNumber)
                        {
                           key["NumberOfBreakdowns"] += 1;
                        }
                    }
                }
            }
